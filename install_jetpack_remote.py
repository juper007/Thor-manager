...
def main() -> None:
    ...
    try:
        ...
        transport.auth_publickey(USER, paramiko.Ed25519Key.from_private_key_file(r"C:\Users\juper\.ssh\id_ed25519"))
        ...
        if status != 0:
            raise RuntimeError(f"JetPack installation failed with exit status {status}")
    finally:
        password = ""
        transport.close()
